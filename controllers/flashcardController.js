...
export const getFlashcards = async (req, res, next) => {
  try {
    // ✅ FIX: 'findOne' ko hata kar 'find' kar diya.
    // Ab ye Document k saare Sets lekar aayega.
    const flashcardSets = await Flashcard.find({
      userId: req.user._id,
      documentId: req.params.documentId
    })
    .populate('documentId', 'title fileName')
    .sort({ createdAt: -1 }); // Naye wale sabse upar

    res.status(200).json({
      success: true,
      count: flashcardSets.length,
      data: flashcardSets // Pura Array wapis bhej rahe hain
    });
  } catch (error) {
    next(error);
  }
